...
const print = async (local, data, status) => {
  let options = {
    html:
      ...
      data +
      '</h3>',
    fileName: 'Comprovante' + data,
    directory: 'Documents',
  };

  let file = await RNHTMLtoPDF.convert(options);
  alert(
    "O comprovante foi baixado e encontra-se na pasta 'Documents' de seus aplicativo",
  );
};
...
export default class MeusAgendamentos extends Component {
  ...
  getMeusAgendamentos = async () => {
    ...
    let url = 'http://192.168.0.113:8000/api/v1/agendamento-vacinacao/';
  ...
  render() {
    return (
      <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
        ...
        {this.state.aguardandoDados && <Text>Meus agendamentos</Text>}
        <DataTable>
          <DataTable.Header>
          ...
          {this.state.meusagendamentos.map((agendamento, index) => (
            <DataTable.Row key={agendamento.agendamento_id}>
              <DataTable.Cell>
                {this.getStatus(agendamento.status)}
              </DataTable.Cell>
              <DataTable.Cell>
                {agendamento.local_vacinacao.nom_estab}
              </DataTable.Cell>
